...
<fieldset>
		...
		{if $action=="edit"}
		 ...
		 {/if}
		  {$smarty.const.LANG_ARTICLES}
		</legend>
<form action="admin.php" method="post" enctype="multipart/form-data">
<table class="list">
     <tr>
		<td class="{cycle values="row1,row2" advance=false}"><strong>{$smarty.const.LANG_TITLE}:</strong> </td>
		<td class="{cycle values="row1,row2" advance=true}"><input type="text" size="35" name="aTitle" value="{$aTitle}" /></td>
	</tr>
	<tr>
		<td class="{cycle values="row1,row2" advance=false}"><strong>{$smarty.const.LANG_PROBLEM}:</strong> </td>
		<td class="{cycle values="row1,row2" advance=true}">
			<textarea id="aShortDesc" name="aShortDesc" style="width: 100%" rows="15">{$aShortDesc}</textarea>
		</td>
	</tr>
	<tr>
		<td class="{cycle values="row1,row2" advance=false}"><strong>{$smarty.const.LANG_SOLUTION}:</strong> </td>
		<td class="{cycle values="row1,row2" advance=true}">
			<script type="text/javascript">if ( typeof tinyMCE == "undefined" || tinyMCE.configs.length < 1 ) edToolbar();</script>
			<textarea id="aDescription" name="aDescription" style="width: 100%" rows="25">{$aDescription}</textarea>
			{if $html_integration!="Y"}
	...
	<tr>
		<td class="{cycle values="row1,row2" advance=false}"><strong>{$smarty.const.LANG_DISPLAY}:</strong> </td>
		<td class="{cycle values="row1,row2" advance=true}"><input type="checkbox" name="aDisplay" value="Y"{if $aDisplay=="N"} {else} CHECKED{/if} /></td>
	</tr>
	<tr>
		<td class="{cycle values="row1,row2" advance=false}"><strong>{$smarty.const.LANG_CATEGORY}:</strong> </td>
		<td class="{cycle values="row1,row2" advance=true}">
			<select name="cat[]" size="10" multiple="multiple">
			{foreach from=$cat item=entry}
			<option value="{$entry.cID}"{if $entry.selected==Y} SELECTED{/if}>{$entry.cName}</option>
			{/foreach}
			</select>
		</td>
	</tr>
	{if $attachments}
	<tr>
		<td class="{cycle values="row1,row2" advance=false}"><strong>{$smarty.const.LANG_ATTACHMENTS}</strong>:</td>
		<td class="{cycle values="row1,row2" advance=true}">
			<table width="100%">
				<tr>
					<td><strong>{$smarty.const.LANG_ATTACHMENTS}</strong></td><td><strong>{$smarty.const.LANG_DELETE}</strong></td>
				</tr>
			{foreach from=$attachments item=entry key=aid}
				<tr>
					<td width="5%" class="{cycle values="row1,row2" advance=false}" nowrap>{$entry.aName}</td><td class="{cycle values="row1,row2" advance=true}"><input type="checkbox" name="attach{$entry.aID}" value="Y" /></td>
				</tr>
			{/foreach}
			</table>
		</td>
	...
	{/if}
	<tr>
		<td class="{cycle values="row1,row2" advance=false}"><strong>{$smarty.const.LANG_ADD_FILES}:</strong> </td>
		<td class="{cycle values="row1,row2" advance=true}">
 ...
 <input type="hidden" name="aAuthor" value="{$uid}" />
 {if $action=="edit"}
