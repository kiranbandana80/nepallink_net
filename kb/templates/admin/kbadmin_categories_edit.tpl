...
{literal}
	function checkform(frm)
	{
		if(frm.cName.value=="")
		{
			alert("Please enter a value in the field \"{/literal}{$smarty.const.LANG_TITLE}{literal}\".");
			frm.cName.focus();
			return (false);
		}
		return (true);
...
<fieldset>
		<legend>
		{if $action=="edit"}
			{$smarty.const.LANG_EDIT}
		{else}
			{$smarty.const.LANG_ADD}
		{/if}
			{$smarty.const.LANG_CATEGORY}
		</legend>
<form action="admin.php" method="post" onsubmit="return checkform(this)">
<table class="list">
	<tr>
		<td class="{cycle values="row1,row2" advance=false}"">{$smarty.const.LANG_TITLE}: </td>
		<td class="{cycle values="row1,row2" advance=true}"><input type="text" size="35" name="cName" value="{$cName}" /></td>
	</tr>
	<tr>
		<td class="{cycle values="row1,row2" advance=false}"">{$smarty.const.LANG_PARENT} {$smarty.const.LANG_CATEGORY}: </td>
		<td class="{cycle values="row1,row2" advance=true}">
			<select name="parent_id">
				<option value="0">{$smarty.const.LANG_NO} {$smarty.const.LANG_PARENT}</option>
				{foreach from=$parent_categories item=entry}
				<option value="{$entry.cID}"{if $entry.cID==$parent_id} SELECTED{/if}>{$entry.cName}</option>
				{/foreach}
			</select>
		</td>
	</tr>
	<tr>
		...
		<td class="{cycle values="row1,row2" advance=true}"><input type="checkbox" name="cDisplay" value="Y"{if $cDisplay=="N"} {else}CHECKED{/if} /></td>
	</tr>
	{if $action=="edit"}
	<tr>
		<td class="{cycle values="row1,row2" advance=false}"">{$smarty.const.LANG_CATEGORY} {$smarty.const.LANG_ORDER}: </td>
		<td class="{cycle values="row1,row2" advance=true}"><input type="text" name="cOrder" value="{$cOrder}" /></td>
	</tr>
	{/if}
	<tr>
		...
		<textarea id="cDescription" name="cDescription" style="width: 100%" rows="15">{$cDescription}</textarea>
		{if $html_integration!="Y"}
			<script type="text/javascript">edCanvas = document.getElementById('cDescription');</script>
			{/if}
		</td>
	</tr>
	<tr>
