...
var pullimage_2025090 = new Image ;
var date = new Date() ;
var unique = dounique() ;
var chat_width = 450 ;
var chat_height = 360 ;
var url = escape( location.toString() ) ;
var phplive_image_2025090 = "http://chat.nepallink.net/image.php?l=admin&x=1&deptid=2&pagex="+url+"&unique="+unique+"&refer="+refer+"&text=" ;
...
style += "<!--" ;
style += "#ProactiveSupport_2025090 {visibility:hidden; position:absolute; height:1; width:1; top:0; left:0;}" ;
style += "-->" ;
style += "</style>" ;
...
function initializeProactive_2025090()
{

	if(!ns && !ie && !w3) return ;
	if(ie)		ProactiveDiv = eval('document.all.ProactiveSupport_2025090.style') ;
	else if(ns)	ProactiveDiv = eval('document.layers["ProactiveSupport_2025090"]') ;
	else if(w3)	ProactiveDiv = eval('document.getElementById("ProactiveSupport_2025090").style') ;
...
function checkinitiate_2025090()
{
	initiate = pullimage_2025090.width ;
	if ( ( initiate == 2 ) && !chat_opened )
	{
		chat_opened = 1 ;
		popblock_action_id = 2 ;
		launch_support_2025090() ;
	}
	else if ( ( initiate == 3 ) && !chat_opened )
	{
		chat_opened = 1 ;
		initializeProactive_2025090() ;
	}
	else if ( initiate == 100 )
	{
		do_tracker_flag_2025090 = 0 ;
	}

	if ( ( initiate == 1 ) && chat_opened )
		chat_opened = 0 ;
}
function do_tracker_2025090()
{
	...
	pullimage_2025090 = new Image ;
	pullimage_2025090.src = "http://chat.nepallink.net/image_tracker.php?l=admin&x=1&deptid=2&pagex="+url+"&unique="+unique ;

	pullimage_2025090.onload = checkinitiate_2025090 ;
	if ( do_tracker_flag_2025090 == 1 )
		setTimeout("do_tracker_2025090()",tracker_refresh) ;
}

function start_timer_2025090( c )
{
	if ( c == 0 )
	{
	...
	{
		--c ;
		var temp = setTimeout( "start_timer_2025090("+c+")", 1000 ) ;
	}
}

function launch_support_2025090()
{
	start_timer_2025090( 2 ) ;
	var request_url_2025090 = "http://chat.nepallink.net/request.php?l=admin&x=1&deptid=2&pagex="+url ;
	var newwin_chat = window.open( request_url_2025090, unique, 'scrollbars=no,menubar=no,resizable=0,location=no,screenX=50,screenY=100,width='+chat_width+',height='+chat_height+'' ) ;
	if ( newwin_chat )
	{
		newwin_chat.focus() ;
		DoClose() ;
...
function WriteChatDiv()
{
	...
	output = "<div id=\"ProactiveSupport_2025090\">" ;
	...
	output += "<a href=\"JavaScript:launch_support_2025090()\" OnMouseOver=\"toggleMotion(1)\" OnMouseOut=\"toggleMotion(0)\"><img src=\""+scroll_image.src+"\" border=0></a>" ;
	output += "</td></tr></table>" ;
	output += "</div>" ;
...
{
	var rejectimage_2025090 = new Image ;
	rejectimage_2025090.src = "http://chat.nepallink.net/image_tracker.php?l=admin&x=1&deptid=2&unique="+unique+"&action=reject" ;
	DoClose() ;
	chat_opened = 0 ;
}

function NotifyPopupBlocker_2025090()
{
	var notifyimage_2025090 = new Image ;
	notifyimage_2025090.src = "http://chat.nepallink.net/image_tracker.php?l=admin&x=1&deptid=2&unique="+unique+"&action=notifypopup" ;
}
