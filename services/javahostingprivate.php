<HTML>
<HEAD>
<TITLE>Java Web Hosting, JSP Hosting, Servlet Hosting, Tomcat Hosting, J2EE Hosting, JSP, Struts Hosting, Nepal Java Hosting</TITLE>
<META http-equiv=Content-Language content=en-us>
<META  name="description" content="NepalLink is the leading provider of Java JSP Web hosting for small & medium businesses.">
<META name="keywords" content="Shared, Dedicated, Managed, Virtual, Business, Web, Hosting, Service, Reseller, Domain,  Internet, E-commerce, Windows, Unix, Linux, FreeBSD, Server, PHP, MySQL, ASP">
<META name="srce" content="http://www.nepallink.net">
<META name="robots" content="index/follow">
<META name="revisit" content="7 days">
<META name="distribution" content="global">
<META HTTP-EQUIV="Content-Type" CONTENT="text/html; charset=iso-8859-1">
<LINK REL="Stylesheet" href="../style.css" type="text/css">
</HEAD>
<style type="text/css">
.mytd1{
	color:#000000;
	background:#E6E6E6;
	text-align:center;
	
	}
.mytd{
	color:#000000;
	background:#EDF4F8;
	text-align:center;
	
}
.mytr{
color:#FFFFFF;
background:#003366;
text-align:center;
font:bold;

}
</style>

<BODY BGCOLOR=#E4E5EA LEFTMARGIN=0 TOPMARGIN=2 MARGINWIDTH=0 MARGINHEIGHT=0 BACKGROUND="../images/bg.gif">
<CENTER>


<table border="0" cellpadding="0" cellspacing="0" width="775">
  <tr>
    <td width="14" align="left" valign="top" background="../images/left.gif">&nbsp;</td>
    <td width="747" align="right" valign="middle" bgcolor="#E5E7E8" height="20">Powerful Private Java Hosting at Affordable Price</td>
    <td width="14" align="left" valign="top" background="../images/right.gif">&nbsp;</td>
  </tr>
</table>


<?php  include_once("../header.php"); ?><table border="0" cellpadding="0" cellspacing="0" width="775">
  <tr>
    <td width="14" align="left" valign="top" background="../images/left.gif"><img src="../images/service/spacer.gif" height="15" width="10" alt=""></td>
    <td width="747" align="left" valign="top" background="../images/bkg.gif"><img src="../images/service/spacer.gif" height="15" width="10" alt=""></td>
    <td width="14" align="left" valign="top" background="../images/right.gif"><img src="../images/service/spacer.gif" height="15" width="10" alt=""></td>
  </tr>
</table>



<table border="0" cellpadding="0" cellspacing="0" width="775">
  <tr>
    <td width="14" align="left" valign="top" background="../images/left.gif">&nbsp;</td>
    <td width="747" align="left" valign="middle" bgcolor="#FFFFFF">



<table border="0" cellpadding="0" cellspacing="0" width="100%">
  <tr>
    <td width="35%" align="left" valign="top"><font class="title">Private Java Hosting Services</font>
	<br>
	<br>
	<ul>
	
 <div align="left"><img src="../images/service/logo_java.gif" alt="Java Hosting, JSP Hosting, Nepal Java Hosting, JSTL Hosting, Java Mysql Hosting"><br>
   NepalLink network offer affordable, quality and efficient Java web hosting. Our Java hosing platform configured for optimum performance using latest Tomcat, MySQL, PostgreSQL, Apache and Web Application frameworks such as Spring, Struts, Hibernate, Cocoon, Ant, JSF, JSTL etc. </div>
 <p>&nbsp;</p>
<script language="JavaScript" src="http://chat.nepallink.net/js/status_image.php?base_url=http://chat.nepallink.net&l=admin&x=1&deptid=2&"></script><br/>
<br>

 <p><a href="../testimonals.php"><b>See!</b></a>, what they says about us!</p>
 <p><a href="../portfolio.php"><b>Visit our portfolio section too!</b></a></p>
	</ul>
    </td>
    <td width="5%" align="left" valign="top" class="rightborder">&nbsp;</td>
    <td width="60%" align="left" valign="top"><img src="../images/service/aboutus.jpg"><br>


<br><br>
<table border="0" cellpadding="0" cellspacing="0" width="100%">
  <!--DWLayoutTable-->
  <tr>
    <td width="45"  >&nbsp;</td>
    <td width="360" align="left" valign="top" >      <div align="justify">
                   <h3>Private Java Hosting </h3>
                   Private Java Hosting package is a streamlined and affordable version of Java package designed for Small Medium Business . It provides you the capability to run full-fledge JSP/Servlets web apps on your own Tomcat instance running on a Private JVM (102MB memory heap). 
                   <p><strong>Browse By: </strong></p>
             <p><a href="#pricing1">Pricing Options </a><br/>
               <a href="#general1">General Features </a><br/>
                <a href="#java1">Java (JSP/Servlets) Features </a><br/>
                <a href="#email1">Email Features</a><br/>
			    <a href="#db1">Database Features</a><br/>
				  <p><a id="pricing1"> <strong>Pricing Options </strong></a> </p> <table cellspacing="2" cellpadding="0" border="0" width="98%">
                      <tr>
                        <td class="mytr" width="60%">NO SETUP FEE </td>
                        <td class="mytr">Yes  </td>
                      </tr>
                      <tr>
                        <td class="mytd"><p>Yearly</td>
                        <td class="mytd1"><p align="center"><strong>USD 200</strong></td>
                      </tr>
                    </table>
                    <p>  <a id="general1"><strong> General Features </strong> </a></p> <table cellspacing="2" cellpadding="0" border="0" width="98%">
                      <tr>
                       <td class="mytr" width="60%">Disk Space  </td>
                        <td class="mytr">Up to 5000 MB *  </td>
                      </tr>
                      <tr>
                        <td class="mytd">High Availability Bandwidth </td>
                        <td class="mytd1"><p align="center">500 Mbps </td>
                      </tr>
                      <tr>
                        <td class="mytd">Advanced Web Based Control Panel </td>
                        <td class="mytd1"><p align="center">Yes </td>
                      </tr>
                      <tr>
                        <td class="mytd">24/7 Emergency Support </td>
                        <td class="mytd1"><p align="center">Yes </td>
                      </tr>
                      <tr>
                        <td class="mytd">Operating System </td>
                        <td class="mytd1"><p align="center">Linux</td>
                      </tr>

  		     <tr>
                        <td class="mytd">Static IPv4</td>
                        <td class="mytd1"><p align="center">1</td>
                      </tr>

                      <tr>
                        <td class="mytd">SSL/HTTPS Support </td>
                        <td class="mytd1"><p align="center">Yes </td>
                      </tr>
                      <tr>
                        <td class="mytd">Frequent &amp; Real-time Backup </td>
                        <td class="mytd1"><p align="center">Yes </td>
                      </tr>
                      <tr>
                        <td class="mytd">Cron Jobs </td>
                        <td class="mytd1"><p align="center">Yes </td>
                      </tr>
                      <tr>
                        <td class="mytd">Web Statistics </td>
                        <td class="mytd1"><p align="center">Yes </td>
                      </tr>
                    </table>
                    <p> <a id="java1"><strong>Java (JSP/Servlets) Features </strong> </a></p> <table cellspacing="2" cellpadding="0" border="0" width="98%">
                      <tr>
                       <td class="mytr" width="60%">Shared Tomcat / Resin  </td>
                        <td class="mytr">Yes  </td>
                      </tr>
                      <tr>
                       <td class="mytd">Private JVM </td>
                      <td class="mytd1">Yes </td>
                      </tr>
                      <tr>
                       <td class="mytd">Private JVM Size </td>
                      <td class="mytd1">1024 MB </td>
                      </tr>


 		    <tr>
                     <td class="mytd">Tomcat version</td>
                    <td class="mytd1">7</td>
                    </tr>

                   <tr>
                     <td class="mytd">JDK/JRE version</td>
                    <td class="mytd1">jdk-8u131</td>
                    </tr>`


                    <tr>
                     <td class="mytd">JVM Initial Heapsize </td>
                    <td class="mytd1">512MB</td>
                    </tr>

                   <tr>
                     <td class="mytd">JVM Max Heapsize </td>
                    <td class="mytd1">1GB</td>
                    </tr>


                      <tr>
                       <td class="mytd">Java Servlets/JSP Support </td>
                      <td class="mytd1">Yes </td>
                      </tr>
                      <tr>
                       <td class="mytd">Can I Start/Stop Tomcat? </td>
                      <td class="mytd1">Yes </td>
                      </tr>
                      <tr>
                       <td class="mytd">Can I run Struts? </td>
                      <td class="mytd1">Yes </td>
                      </tr>
                      <tr>
                       <td class="mytd">Can I run Hibernate? </td>
                      <td class="mytd1">Yes </td>
                      </tr>
                      <tr>
                       <td class="mytd">Can I run Spring Framework? </td>
                      <td class="mytd1">Yes </td>
                      </tr>
                      <tr>
                       <td class="mytd">Can I run other Java based frameworks? </td>
                      <td class="mytd1">Yes </td>
                      </tr>
                      <tr>
                       <td class="mytd">Can I send mail through my application? </td>
                      <td class="mytd1">Yes </td>
                      </tr>
                    </table>
                    <!-- <p>  <a id="email1"><strong> Email Features </strong></a> </p> <table cellspacing="2" cellpadding="0" border="0" width="98%">
                      <tr>
                       <td class="mytr" width="60%">POP3 accounts  </td>
                       <td class="mytr">Unlimited  </td>
                      </tr>
                      <tr>
                        <td class="mytd">E-mail Aliases </td>
                       <td class="mytd1">Unlimited </td>
                      </tr>
                      <tr>
                        <td class="mytd">E-mail Auto Responder </td>
                       <td class="mytd1">Unlimited </td>
                      </tr>
                      <tr>
                        <td class="mytd">Unlimited E-mail Forwarding </td>
                       <td class="mytd1">Unlimited </td>
                      </tr>
                      <tr>
                        <td class="mytd">Catch all e-mail addresses </td>
                       <td class="mytd1">Unlimited </td>
                      </tr>
                      <tr>
                        <td class="mytd">Email Spam Filter </td>
                       <td class="mytd1">Yes </td>
                      </tr>
                      <tr>
                        <td class="mytd">Virus Scan </td>
                       <td class="mytd1">Yes </td>
                      </tr>
                      <tr>
                        <td class="mytd">Web Mail </td>
                       <td class="mytd1">Yes </td>
                      </tr>
                    </table> -->
                    <p>  <a id="db1"> <strong>Database Features </strong> </a></p> <table cellspacing="2" cellpadding="0" border="0" width="98%">
                      <tr>
                        <td class="mytr" width="60%">PHPMyAdmin  </td>
                        <td class="mytr">Yes  </td>
                      </tr>
                      <tr>
                        <td class="mytd"><p>MySQL Databases </td>
                        <td class="mytd1"><p align="center">10 </td>
                      </tr>
                      <tr>
                        <td class="mytd"><p>PostgreSQL Databases </td>
                        <td class="mytd1"><p align="center">10 </td>
                      </tr>
                      <tr>
                        <td class="mytd"><p>Can I use MySQL Connection Center </td>
                        <td class="mytd1"><p align="center">Yes </td>
                      </tr>
                      <tr>
                        <td class="mytd"><p>Can I connect from local desktop? </td>
                        <td class="mytd1"><p align="center">Yes </td>
                      </tr>
                    </table>    
                    <br>
                    <br>
                    <p align="center">
					  <a href="http://nepallink.net/order.php?plan=JAVA_PRIVATE_HOSTING"><img src="http://nepallink.net/images/order_blue.gif" alt="Order Now Java Hosting, India Java Hosting, JSP Hosting" border="0"></a>
					</p>
                    
					</div></td>
    <td width="45" >&nbsp;</td>
  </tr>
</table>


<br>


<ul>
<div align="left"></div>
	</ul>	</td>
  </tr>
</table>


<p>&nbsp;</p>

</td>
    <td width="14" align="left" valign="top" background="../images/right.gif">&nbsp;</td>
  </tr>
</table>
<?php  include_once("../footer.php"); ?>
